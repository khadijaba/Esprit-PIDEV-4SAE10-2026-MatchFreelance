...
class AnalyzeRequest(BaseModel):
    ...
    prefer_llm: bool = Field(
        False,
        alias="preferLlm",
        description="Si True, tente le LLM (OpenAI/Ollama) quand configuré, sinon retombe sur les règles.",
    )
...
class AnalyzeResponse(BaseModel):
    ...
    analysis_source: str = Field(
        "rules",
        alias="analysisSource",
        description="rules | llm — origine de l'analyse structurée",
    )
...
# --- Notification automatique compatibilité projet ---
class ComputeCompatibleRequest(BaseModel):
    project_id: int = Field(..., alias="projectId", description="ID du projet publié")
    project_title: str = Field(..., alias="projectTitle", min_length=1)
    required_skills: list[str] = Field(default_factory=list, alias="requiredSkills")
...
class CompatibleFreelancerNotification(BaseModel):
    freelancer_id: int = Field(..., alias="freelancerId")
    full_name: Optional[str] = Field(None, alias="fullName")
    email: Optional[str] = None
    score: int = Field(..., ge=0, le=100, description="Score de compatibilité 0-100")
    subject: Optional[str] = Field(None, description="Sujet de la notification")
    message: str = Field(..., description="Message à afficher / envoyer au freelancer")

    class Config:
        populate_by_name = True


class ComputeCompatibleResponse(BaseModel):
    notifications: list[CompatibleFreelancerNotification] = Field(
        default_factory=list,
        description="Liste des freelancers avec score >= seuil (ex. 70%), prêts pour envoi",
    )
    threshold: int = Field(70, description="Seuil de score utilisé")
