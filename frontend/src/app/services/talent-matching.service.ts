...
export class TalentMatchingService {
  ...
  private computeMatching(required: string[], allSkills: Skill[], users: User[], limit = 5): MatchingFreelancer[] {
    ...
    for (const s of allSkills) {
      const fid = Number(s.freelancerId);
      if (!Number.isFinite(fid)) continue;
      if (!byFreelancer.has(fid)) byFreelancer.set(fid, []);
      byFreelancer.get(fid)!.push(s);
    }

    const results: MatchingFreelancer[] = [];
    // Parcourir tous les comptes FREELANCER (pas seulement ceux qui ont déjà des lignes en SKILL),
    // sinon un compte sans compétences enregistrées n’apparaît jamais dans le Top 5.
    for (const user of users) {
      const fid = Number(user.id ?? user.userId);
      if (!Number.isFinite(fid)) continue;
