...
export function normalizeProjectRequiredSkills(project: Project): Project {
  const fromField = normalizeRequiredSkills(project.requiredSkills);
  let required: string[];
  if (fromField.length > 0) {
    // Compétences saisies explicitement : ne pas les filtrer par la taxonomie (sinon liste vide côté matching).
    required = [...new Set(fromField.map((s) => s.trim()).filter((s) => s.length >= 2))];
