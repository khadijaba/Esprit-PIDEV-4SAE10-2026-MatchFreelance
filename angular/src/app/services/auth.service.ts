...
export class AuthService {
    ...
    private apiUrl = `${this.userMicroserviceUrl}/auth`;
    ...
    loginWithFace(request: { email: string; faceDescriptor: string }): Observable<JwtResponse> {
        return this.http.post<JwtResponse>(`${this.apiUrl}/signin-face`, request).pipe(
            tap(response => {
                if (response && response.token) {
                    this.setSession(response);
                }
