...
| `TEAM_AI_BACKEND_URL` | URL du backend (Gateway) pour récupérer skills et users (défaut : http://localhost:8086) |
...
### `POST /api/compute-compatible-freelancers` (notification automatique de compatibilité)

Quand un client **publie un nouveau projet**, le frontend appelle cet endpoint pour calculer les freelancers dont le **score de compatibilité est ≥ 70%** (compétences, expérience, note, projets réalisés). Les freelancers reçoivent ensuite une **notification in-app** (et optionnellement par email si le backend l’implémente).

Corps JSON :
```json
{
  "projectId": 42,
  "projectTitle": "Développement Web E-Commerce",
  "requiredSkills": ["Java", "React", "Spring Boot"]
}
```

Réponse :
- `notifications` : liste de { freelancerId, fullName, email, score, subject, message }
- `threshold` : seuil utilisé (70)

Le service Python récupère **skills** et **users** (rôle FREELANCER) depuis le backend (variable `TEAM_AI_BACKEND_URL`, défaut `http://localhost:8086`). Algorithme aligné avec le frontend (TalentMatchingService).
...
│       └── compatibility_notifier.py  # Compatibilité projet → notifications (score ≥ 70%)
├── requirements.txt
├── run.py
└── README.md
```
