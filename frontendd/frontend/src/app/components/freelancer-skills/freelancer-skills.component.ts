...
export class FreelancerSkillsComponent implements OnInit {
  ...
  /** Niveau avant édition (pour enregistrer l'évolution après update) */
  previousLevelWhenEditing: string | undefined;
  ...
  openEditSkillForm(skill: Skill) {
    ...
    this.previousLevelWhenEditing = skill.level ?? undefined;
    this.skillForm = {
      name: skill.name,
  ...
  cancelSkillForm() {
    ...
    this.editingSkillId = null;
    this.previousLevelWhenEditing = undefined;
  }
  ...
  addSkill() {
    ...
    if (this.editingSkillId != null) {
      const newLevel = this.skillForm.level ?? '';
      const previousLevel = this.previousLevelWhenEditing;
      this.skillService.update(this.editingSkillId, this.skillForm).subscribe({
        next: () => {
          if (newLevel && previousLevel !== newLevel) {
            this.skillEvolutionService.record(
              this.freelancerId,
              this.editingSkillId!,
              this.skillForm.name,
              newLevel,
      ...
      const newLevel = this.skillForm.level ?? 'BEGINNER';
