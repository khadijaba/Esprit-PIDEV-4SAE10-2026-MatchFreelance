...
import com.freelancing.candidature.dto.ApplicationCountDTO;
...
public class CandidatureController {
    ...
    /** Comptage léger pour analytics (même contrôle propriétaire que la liste). */
    @GetMapping("/project/{projectId}/application-count")
    public ResponseEntity<?> countApplicationsByProject(
            @PathVariable Long projectId,
            @RequestParam Long clientId) {
        try {
            long n = candidatureService.countApplicationsForProject(projectId, clientId);
            return ResponseEntity.ok(new ApplicationCountDTO(n));
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(Collections.singletonMap("message", safeErrorMessage(e)));
        }
