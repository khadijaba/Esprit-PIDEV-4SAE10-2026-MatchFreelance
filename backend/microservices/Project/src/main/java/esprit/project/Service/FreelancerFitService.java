...
public class FreelancerFitService {
    ...
    public FreelancerFitBatchDto computeFit(Long projectId, List<Long> freelancerIds) {
        ...
        for (Long fid : freelancerIds.stream().distinct().toList()) {
            try {
                ...
                log.warn("freelancer-fit computeOne failed projectId={} freelancerId={}: {}", projectId, fid, ex.toString());
                rows.add(minimalFallbackFit(fid));
            }
        }
    ...
    private FreelancerFitDto computeOne(Project target, Long freelancerId) {
        ...
        Double avgClientRating = safeAverageRating(freelancerId);
        ...
        int competencesPercent = (int) Math.round(clamp(skillFitTarget, 0, 100));
        int experiencePercent = (int) Math.round(clamp(experience, 0, 100));
        int weightedMatchPercent = (int) Math.round(clamp(0.7 * competencesPercent + 0.3 * experiencePercent, 0, 100));
        ...
        return FreelancerFitDto.builder()
                ...
                .competencesPercent(competencesPercent)
                .experiencePercent(experiencePercent)
                .weightedMatchPercent(weightedMatchPercent)
    ...
    /**
     * Évite un HTTP 500 si la table {@code client_freelancer_rating} est absente ou la requête AVG échoue.
     */
    private Double safeAverageRating(Long freelancerId) {
        if (freelancerId == null) {
            return null;
        }
        try {
            return ratingRepository.averageRatingByFreelancerId(freelancerId);
        } catch (Exception ex) {
            log.debug("averageRatingByFreelancerId skipped for {}: {}", freelancerId, ex.getMessage());
            return null;
        }
    }

    private static FreelancerFitDto minimalFallbackFit(Long freelancerId) {
        return FreelancerFitDto.builder()
                .freelancerId(freelancerId)
                ...
                .durationHighDays(45)
                .successScore(0)
                .competencesPercent(0)
                .experiencePercent(0)
                .weightedMatchPercent(0)
                .confidence("LOW")
                .pastMissionsConsidered(0)
                .summary("Calcul temporairement indisponible pour ce profil (vérifiez les logs du microservice Project).")
                .build();
    }
